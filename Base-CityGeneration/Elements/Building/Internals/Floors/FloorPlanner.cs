...
namespace Base_CityGeneration.Elements.Building.Internals.Floors
{
    internal class FloorPlanner
    {
        ...
        {
            if (_used)
                throw new InvalidOperationException("Cannot used the same FloorPlanner twice");
            _used = true;
        ...
        private void LayoutRegion(FloorplanRegion region)
        {
            ...
            var spaces = region.LayoutSpaces(_random, _metadata);
            ...
            EnsureConnectivity(region, spaces);
            ...
            foreach (var room in rooms)
            {
                ...
                _plan.AddRoom(RoomShape(room.Key, region.OABR), _wallThickness, new[] { scripts.HasValue ? scripts.Value.Script : null });
            }
            ...
            foreach (var group in groups)
            {
                ...
                var shape = _plan.TestRoom(RoomShape(group.Key, region.OABR), shrink: false);
                if (!shape.Any())
        ...
        private static void EnsureConnectivity(FloorplanRegion region, IEnumerable<KeyValuePair<BoundingRectangle, BaseSpaceSpec>> spaces)
        {
